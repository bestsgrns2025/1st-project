...
import ImageManagement from './ImageManagement'; // Import ImageManagement
import ContentManagement from './ContentManagement'; // Import ContentManagement
...
const AdminDashboard = () => {
  ...
  const [selectedTab, setSelectedTab] = useState('inquiries'); // Add state for selected tab
  ...
  }, [navigate, toast, selectedTab]); // Add selectedTab to dependencies
  ...
  if (loading && selectedTab === 'inquiries') { // Only show loading for inquiries initially
    return (
  ...
  return (
    <div className="min-h-screen bg-background text-foreground py-8 px-4 sm:px-6 lg:px-8">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="mb-8 border-b border-gray-700">
          <nav className="-mb-px flex space-x-8" aria-label="Tabs">
            <button
              ...
              className={`whitespace-nowrap py-4 px-1 border-b-2 font-medium text-sm ${selectedTab === 'inquiries' ? 'border-primary text-primary' : 'border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300'}`}
            >
              Inquiries
            </button>
            <button
              onClick={() => setSelectedTab('image-management')}
              className={`whitespace-nowrap py-4 px-1 border-b-2 font-medium text-sm ${selectedTab === 'image-management' ? 'border-primary text-primary' : 'border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300'}`}
            >
              Image Management
            </button>
            <button
              onClick={() => setSelectedTab('content-management')}
              className={`whitespace-nowrap py-4 px-1 border-b-2 font-medium text-sm ${selectedTab === 'content-management' ? 'border-primary text-primary' : 'border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300'}`}
            >
              Content Management
            </button>
            <button
              onClick={() => setSelectedTab('product-management')}
              className={`whitespace-nowrap py-4 px-1 border-b-2 font-medium text-sm ${selectedTab === 'product-management' ? 'border-primary text-primary' : 'border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300'}`}
            >
              Product Management
            </button>
          </nav>
        ...
        {selectedTab === 'image-management' && <ImageManagement />}

        {selectedTab === 'content-management' && <ContentManagement />}

        {selectedTab === 'product-management' && <ProductManagement />}
      </div>
    </div>
  );
};
